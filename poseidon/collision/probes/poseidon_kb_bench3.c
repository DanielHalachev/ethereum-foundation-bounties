...
static inline uint32_t reduce64(uint64_t x) {
    /* Barrett reduction for x < 2^42 (our MDS accumulator), p ~ 2^31.
     * m = floor(2^64 / p). q ~= (x*m) >> 64 underestimates x/p by <=1. */
    const uint64_t M = 8657571868ULL;          /* floor(2^64 / p) */
    uint64_t q = (uint64_t)(((__uint128_t)x * M) >> 64);
    uint64_t r = x - q * P;
    if (r >= P) r -= P;                          /* at most one correction */
    return (uint32_t)r;
}
...
int main(int argc, char** argv) {
    ...
    printf("KAT OK (v3 Barrett-reduced MDS).\n");
